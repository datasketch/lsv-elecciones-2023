...
function Modal() {
  ...
  const getConvictedOrInvestigated = () => {
    if (
      featuredCandidate.hasBeenConvicted === 'No' &&
      featuredCandidate.hasBeenInvestigated === 'No'
    )
      return 'No';
    else if (
      featuredCandidate.hasBeenInvestigated === 'Sí' &&
      featuredCandidate.hasBeenConvicted === 'Sí'
    )
      return 'Ambas';
    else if (featuredCandidate.hasBeenInvestigated === 'Sí')
      return 'Investigado';
  ...
  const getHeirHistory = () => {
    if (
      featuredCandidate.heirToDoomedVows === 'No' &&
      featuredCandidate.heirToVotesUnderInvestigation === 'No'
    )
      return 'No';
    else if (
      featuredCandidate.heirToDoomedVows === 'Sí' &&
      featuredCandidate.heirToDoomedVows === 'Sí'
    )
      return 'Ambas';
    else if (featuredCandidate.heirToDoomedVows === 'Sí') return 'De condenado';
    else if (featuredCandidate.heirToVotesUnderInvestigation === 'Sí')
      return 'De investigado';
  };

  const redflags = [
    featuredCandidate.firstRedflag,
    featuredCandidate.secondRedflag,
    featuredCandidate.thirdRedflag,
  ].filter((rf) => rf);

  return (
    <div
      className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 py-8 pr-80 flex items-center justify-center z-20"
      onClick={handleClose}
    >
      <div className="bg-white w-96 max-h-full overflow-auto">
        <div className="flex justify-between items-start">
          <div
            className="font-martin w-32 uppercase text-center text-2xl text-white py-2 px-4"
            style={{ backgroundColor: featuredCandidate.color }}
          >
          ...
          <button className="close px-4 self-center">
            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14">
              ...
              <path
                data-name="Trazado 2"
                d="M14 1.478 1.478 14 0 12.522 12.522 0Z"
                fill="#3d3b3d"
              />
            </svg>
          </button>
        ...
        <div className="px-4 flex items-start">
          ...
          <div className="flex-grow pb-4 border-b border-bone mx-3.5">
            <div className="flex justify-between items-center text-xs">
              <span>{featuredCandidate.party.label}</span>
              <span className="text-dodger-blue border-r border-dodger-blue pr-2">
                {featuredCandidate.department}
              </span>
            </div>
        ...
        {featuredCandidate.profile && (
          <p className="bg-cultured py-4 px-8 text-sm">
            {featuredCandidate.profile}
          </p>
        )}
        {!!redflags.length && (
